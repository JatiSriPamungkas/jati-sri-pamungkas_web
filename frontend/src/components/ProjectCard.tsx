...
const ProjectCard = ({ urlImage, title, children, techTags }: ProjectCardSchema) => {
	return (
		<>
			<div className="my-16 flex flex-col gap-8 p-8 w-150 min-h-150 rounded-[10px] shadow-[4px_4px_15px_3px_rgba(0,0,0,0.25)]">
				<img src={urlImage} alt="Task Master" className="rounded-[10px]" />
				<div className="flex items-center justify-between">
					<h1 className="font-bold text-3xl ">{title}</h1>
					<ExternalLink className="!w-10 !h-10 text-jati-1" />
				</div>
				<p>{children}</p>
